...
app.use(
  cors({
    origin: ['https://coffee-store-one.web.app'],
    credentials: true
  })
  );


const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.c4v4qwp.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.put('/coffees/:id', async(req, res) => {
      ...
      const result = await coffeeCollections.updateOne(filter, updateInfo, options);
      res.send(result);
...
app.get('/', (req, res) => {
    res.send('Hello miss you my sweet heart!');
})
app.listen(port, () => {
